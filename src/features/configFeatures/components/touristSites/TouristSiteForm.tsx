...
import { SitioTuristico, SitioTuristicoFormData } from './services/TouristSiteService';

interface TouristSiteFormProps {
  formMethods: UseFormReturn<SitioTuristicoFormData>;
  mode: 'create' | 'edit';
  sitios: SitioTuristico[];
  imageUrls: string[];
  onSubmit: (data: SitioTuristicoFormData) => void;
...
const TouristSiteForm: React.FC<TouristSiteFormProps> = ({
  formMethods,
  mode,
  sitios,
  imageUrls,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        ...
        <select
          {...register('ubiccodiValue', { required: 'La ubicación es requerida', valueAsNumber: true })}
          className="w-full px-3 py-2 border text-black border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500"
        >
          <option value="">Seleccione una ubicación</option>
          {sitios.map((sitio) => (
            <option key={sitio.ubiccodi.ubiccodi} value={sitio.ubiccodi.ubiccodi}>
...
export default TouristSiteForm;